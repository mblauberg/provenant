...
Use this reference when the forum is an Australian tribunal rather than a court:
the Commonwealth merits review body and the state and territory civil and
administrative tribunals. Load `forum-and-document-recipes.md` for the forum
adapter and document functions, and `argument-structure-and-paragraphing.md` for
the argument itself. This file supplies no current law, jurisdiction map, form
or time limit.

## Identify The Body, The Division And The Enabling Act

A tribunal's name is not enough to fix its procedure. Record:

- the tribunal, and the division, list or stream that hears this matter;
- the enabling Act that gives the tribunal its power, and the substantive Act
  under which the original decision was made;
- whether the matter is an original application, an internal review, an external
  review of an agency decision, or an appeal within the tribunal;
- the time limit for starting, and whether an extension is available;
- whether an internal review or other precondition must happen first;
- the tribunal's rules, practice notes and directions for this list; and
...
| Feature | Usual tribunal position | Drafting consequence |
|---|---|---|
...
Merits review asks what the correct or preferable decision is on the material
before the tribunal. That is not the same as showing that the original
...
| Draft orders or directions | States the disposition sought in operative terms |

Keep facts in the facts document and argument in the contentions or submissions.
A statement of facts and contentions that blends the two invites the other side
to dispute the whole document.

## Drafting For A Tribunal Member

- Lead with the outcome sought and the criteria in issue.
- Number paragraphs and keep one proposition each.
- Give exact page or paragraph pinpoints into the bundle, using the tribunal's
  pagination.
- Address the agency's or respondent's stated reasons directly, rather than
  restating the client's history.
- Concede what should be conceded early, then narrow to what is genuinely in
  dispute.
- Keep the register plain. Ceremonial advocacy reads worse in a tribunal than in
  a court, and it hides the criteria.

## After The Decision

Appeal routes from tribunals are narrow and body-specific. Some allow an
internal appeal on fact and law, others allow an appeal to a court on a question
of law only, and time limits are short. Verify the route, the time limit and the
correct court from current official sources, and obtain qualified review before
advising on it.

## Completion Gate

Confirm the division's rules, practice notes and directions, confirm the
document names and forms the list uses, check pinpoints against the tribunal's
own pagination, confirm the disposition sought is within power, and keep filing
and service as separately authorised acts.
