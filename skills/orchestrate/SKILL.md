...
3. For writers, pass `mode: "worktree_write"` and the registered `worktree`, named per `setup-repo`'s branch naming doctrine; the task id defaults to that same branch-derived string.
