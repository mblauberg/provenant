"""Normalise digest-verified worker evidence for the review ladder."""
...
def _terminal_verdict(terminal_result: object) -> tuple[str, str]:
    ...
    if verdict == "EXECUTION-UNAVAILABLE":
...
def normalise_dispatch_review(
    ...
    *,
    ...
    """Return the small, derived leg shape consumed by ``check_review_ladder``."""
    ...
    terminal_available = (
        ...
        and dispatch_result.get("terminal_observed") is True
    )
    ...
    elif lineage_reason.startswith("endpoint/provider identity") or lineage_reason == "orchestrator-family-mismatch":
        leg_status = "failed"
        reason = lineage_reason
    elif status != "ok":
        leg_status = "failed"
