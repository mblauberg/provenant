...
These are strict contract probes rather than a broad regression list. Keep
them aligned with the live product/instance boundary as that contract evolves.
"""
...
FUSED_SKILL_ROOT_COMMAND = re.compile(
    r"(?:\$\{AGENTS_HOME:-\$HOME/\.agents\}|~?/\.agents|\$HOME/\.agents)/skills/"
)
...
def test_model_route_treats_an_empty_instance_root_as_unset(tmp_path: Path) -> None:
    default_instance = tmp_path / "home" / ".agents"
    config = default_instance / "config"
    config.mkdir(parents=True)
    catalogue = json.loads((ROOT / "config" / "model-routing.json").read_text())
    ...
    (config / "model-routing.json").write_text(json.dumps(catalogue))
    shutil.copy2(ROOT / "config" / "model-preferences.json", config / "model-preferences.json")
    environment = _clean_split_environment(tmp_path, ROOT)
    environment["AGENT_FABRIC_INSTANCE_ROOT"] = ""
    environment["AGENT_FABRIC_PRODUCT_ROOT"] = ""
    ...
    for relative in ("skills", "agents", "workflows"):
        for path in sorted((ROOT / relative).rglob("*")):
            ...
            for line_number, line in enumerate(path.read_text(errors="replace").splitlines(), start=1):
                if FUSED_SKILL_ROOT_COMMAND.search(line):
