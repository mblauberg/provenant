...
def test_review_topology_binds_explicit_codex_route_and_review_evidence(tmp_path):
    ...
    route.write_text(json.dumps({
        ...
        "read_only_guarantee": "enforced",
        "route_alias": "flagship", "reviewer_id": "codex-primary",
    }))
...
def test_review_topology_rejects_malformed_fields_and_symlink_escape(tmp_path):
    ...
    route.write_text(json.dumps({
        "status": "ok", "tool": "codex", "adapter": "codex", "adapter_gate": "direct-cli",
        "resolved_model": "gpt-test",
        "catalog_model": "", "model_family": "openai",
        "exit": 0, "output_path": str(terminal),
        "provider_family": "openai", "endpoint_provider": "openai", "orchestrator_family": "openai",
        "output_digest": "sha256:" + __import__("hashlib").sha256(terminal.read_bytes()).hexdigest(),
        "read_only_guarantee": "enforced",
        "route_alias": "workhorse", "reviewer_id": "escaped",
    }))
