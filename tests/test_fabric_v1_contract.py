...
STATUSES = {'ok','partial','failed','usage_limited','rate_limited','auth_required','model_unavailable','permission_blocked','stalled','timed_out','cancelled','interrupted','rejected','tool_missing','input_required'}


def validate_attempt(row):
    required = {'schema','run_id','task_id','attempt','state','status','mode','cwd','worktree','started_at','ended_at','last_progress_at','pgid','session_id','retryable','reset_at','retry_after','fix','evidence','question','applied','warnings','provenance','paths','digest'}
    ...
    assert row['status'] in STATUSES if row['state'] == 'terminal' else row['status'] is None
    ...
    assert {'requested','resolved_model','observed_model','observed_source','identity','provider','transport','family','effort_requested','effort_applied','cli_version','fallback_from','notes','line'} <= provenance.keys()
...
def test_shared_contract_examples():
    ...
    for key, item in cooldowns['cooldowns'].items():
        ...
        assert {'account','status','cooling_until','source_run','recorded_at','signature'} <= item.keys()
    assert {'run_id','task_id','attempt','dir','status','line','result_sha256','ended_at'} <= read('index.json').keys()
    assert read('resume.json')['resume'] == read('attempt.json')['run_id']
    assert len(read('layout-cases.json')) == 3
