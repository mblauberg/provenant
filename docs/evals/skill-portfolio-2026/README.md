...
|---|---:|---|---|
| [Current 31-owner holdout](routing-holdout.yaml) | 18 cases x 2 families x 3 trials | Not run | Catalogue holdout only; no current evaluation protocol has been frozen. |
...
repository. Its 31-owner count is checked against `skills/*/SKILL.md` by the
...
current run must freeze a 31-owner protocol for the direct-provider batch path
...
do not establish current 31-owner routing behaviour.
