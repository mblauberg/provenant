...
|---|---:|---|---|
| [Current 27-owner holdout](routing-holdout.yaml) | 18 cases x 2 families x 3 trials | Not run | Catalogue holdout only; no current evaluation protocol has been frozen. |
...
repository. Its 27-owner count is checked against `skills/*/SKILL.md` by the
...
current run must freeze a 27-owner protocol for the direct-provider batch path
...
do not establish current 27-owner routing behaviour.
