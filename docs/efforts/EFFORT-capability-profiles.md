# EFFORT: capability-compiled execution authority

## Destination

This is a historical route map. [ADR 0002](../adr/0002-capability-compiled-execution-authority.md)
was superseded by [ADR 0020](../adr/0020-retire-the-daemon-fabric.md); the
specifications below are not current owners. GitHub work items retain their
historical disposition. Current Fabric ownership is
[`runtime/fabric/README.md`](../../runtime/fabric/README.md).

## Route

- [Programme issue #23](https://github.com/mblauberg/provenant/issues/23)
- Authority specification
- Workspace-containment specification
- Provider-actions specification
- Provider-write specification
- [Lifecycle-authority issue #118](https://github.com/mblauberg/provenant/issues/118)
- [Authority-envelope issue #123](https://github.com/mblauberg/provenant/issues/123)
- [Provider-write issue #129](https://github.com/mblauberg/provenant/issues/129)
- [Provider-write PR #133](https://github.com/mblauberg/provenant/pull/133)
- [Runtime-control issue #134](https://github.com/mblauberg/provenant/issues/134)
- [Runtime-control PR #137](https://github.com/mblauberg/provenant/pull/137)
- [Startup issue #136](https://github.com/mblauberg/provenant/issues/136)
- [Startup PR #138](https://github.com/mblauberg/provenant/pull/138)
- [Custody issue #125](https://github.com/mblauberg/provenant/issues/125)
- [Receipt-history issue #128](https://github.com/mblauberg/provenant/issues/128)
- [Generation-loss issue #131](https://github.com/mblauberg/provenant/issues/131)
- [Git-provenance issue #132](https://github.com/mblauberg/provenant/issues/132)
- [Bootstrap issue #139](https://github.com/mblauberg/provenant/issues/139)

## Invariants

- [ADR 0002](../adr/0002-capability-compiled-execution-authority.md)
- [ADR 0011](../adr/0011-github-owns-work-state.md)
- [Harness constitution](../../HARNESS.md)
