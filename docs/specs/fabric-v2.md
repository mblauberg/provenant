...
|---|---|
| `fabric_dispatch` | One top-level `prompt` or `prompt_file`, `tasks[]` (1–64, `concurrency` 1–8), or `resume` with a new prompt. Optional route and control fields include `adapter`, `alias`, `model`, `effort`, `mode`, `worktree`, `cwd`, `network`, `sandbox`, `add_dirs`, `fallback`, `task_id`, positive finite `timeout_seconds`, `wait_seconds` (non-negative integer; values above 55 clamp to 55 with a warning), and `detail`. Concurrency above 8 clamps to 8 with a warning. |
