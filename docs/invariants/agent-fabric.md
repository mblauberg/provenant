...
| Replaying an already-marked legacy generation does not rewrite its marker | `markLegacyBootstrapSeatGeneration` returns before writing when the marker already records that generation | `zero-touch-lifecycle-receipt.test.ts` |
...
| A stale protocol build is classified by `doctor`, not merely declared unobservable | the `protocol-build` check runs the shared preflight, so the package bin and in-process callers reach it; `wrapperIntercepted` records only that `scripts/agent-fabric` exits 78 first | `status-cli.unit.test.ts` |
