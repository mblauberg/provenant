...
| Every automatic lifecycle action appears in one machine-readable receipt | `bootstrapMcpSeat` returns a typed action receipt covering trust, daemon, seat generation and the bounded identity/mailbox smoke; the CLI and the `fabric_bootstrap` tool relay it | `zero-touch-lifecycle-receipt.test.ts` |
| A converged repeat invocation mutates nothing | the receipt reports no mutating action, the installed generation bytes and inode timestamps are unchanged, and the custody row counts and daemon PID are unchanged | `zero-touch-lifecycle-receipt.test.ts` |
| The identity/mailbox health smoke terminates on its own deadline | a whole-smoke wall-clock budget covers connect, `whoami` and `mailbox.read`; expiry reports a failed smoke instead of hanging or discarding the receipt | `zero-touch-lifecycle-receipt.test.ts` |
| `doctor` names why a lifecycle state holds, not only which state it is | every check declares its precondition; `cause` names the deciding check, and only a converging transition or unreachable-daemon residue is `recovering`, everything else `blocked` | `status-cli.unit.test.ts` |
| Material state displacement is one user decision reported with counts | a schema mismatch raises a typed gate carrying the retained-work census, consequences and the exact `database archive-and-fresh` command, and displaces nothing | `zero-touch-lifecycle-receipt.test.ts` |
