...
import { OWNER_RECORD_NAME, terminateRecordedRun, type RecordedRun } from "../src/run-registry.js";
...
it("a closure write that fails for another reason keeps the owner record", async () => {
  const root = mkdtempSync(join(tmpdir(), "fabric-closure-"));
  const runDir = join(root, "blocked-run");
  mkdirSync(runDir);
  writeFileSync(join(runDir, OWNER_RECORD_NAME), "{}\n");
  // A directory where the temporary status file goes: the write fails with
  // EISDIR while the owner record itself stays removable.
  mkdirSync(join(runDir, `dispatch-status.json.${process.pid}.tmp`));
  try {
    const run = {
      schema_version: 1, kind: "dispatch", run_dir: runDir, workspace: root, run_token: "t",
      owner_pid: 2_147_483_000, owner_pgid: 2_147_483_000, owner_started_at: null,
