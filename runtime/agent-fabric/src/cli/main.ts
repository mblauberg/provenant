...
const DOCTOR_USAGE = `usage: agent-fabric doctor [--consume-provider-quota] [--project PATH] [--agents-home PATH] [--product-root PATH] [--instance-root PATH] [--trusted-config PATH] [--compatibility PATH] [--compatibility-schema PATH] [--review-profile EXACT_DEPLOYED_PATH] [--json]
...
async function main(arguments_: string[]): Promise<void> {
  ...
  if (arguments_[0] === "adapter" && arguments_[1] === "invocation") {
    const { resolveAdapterInvocationCli } = await import("./adapter-invocation.js");
    process.stdout.write(`${JSON.stringify(await resolveAdapterInvocationCli(arguments_.slice(2)))}\n`);
    return;
  }
  ...
  if (arguments_[0] === "mcp" && arguments_[1] === "peer-provision") {
    const [{ provisionMcpPeerSeats }, { resolveFabricPaths }] = await Promise.all([
  ...
  if (arguments_[0] === "database" && arguments_[1] === "archive-and-fresh") {
    const [{ runDatabaseArchiveAndFreshCli }, { resolveFabricPaths }] = await Promise.all([
      import("./database-archive-and-fresh-cli.js"),
      import("./paths.js"),
    ]);
