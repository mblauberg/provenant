...
type TrustMode = "automatic" | "explicit";
type Fixture = { projectRoot: string; paths: FabricPaths; agentsHome: string };

async function fixture(trustMode: TrustMode = "explicit"): Promise<Fixture> {
...
async function makeLegacyTrustRegistry(value: Fixture, trustMode: TrustMode): Promise<{
  ...
  if (entry === undefined) throw new Error("legacy trust entry is missing");
...
describe("zero-touch lifecycle action receipt", () => {
  ...
  it.each([
    ...
    async (trustMode, expectedOutcome) => {
      ...
      await expect(runWorkspaceTrust(["status", value.projectRoot], value.paths)).resolves.toMatchObject({
        trusted: true,
        entry: { establishmentKind: trustMode === "automatic" ? "automatic-bootstrap" : "local-operator" },
      });
      ...
      expect(action(replay.receipt, "seat-generation")).toMatchObject({
        outcome: "replayed",
        mutated: false,
        generation: first.generation,
      });
      ...
      const canonical = JSON.parse(await readFile(join(value.paths.stateDirectory, "trusted-workspaces.json"), "utf8")) as {
        entries: Record<string, unknown>[];
      };
      ...
      try {
        expect(database.prepare("SELECT trust_record_digest FROM projects WHERE canonical_root=?")
          .get(value.projectRoot)).toMatchObject({ trust_record_digest: action(replay.receipt, "workspace-trust").trustRecordDigest });
      } finally {
        database.close();
      }
    },
